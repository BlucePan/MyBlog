...
<article class="blogs">
<h1 class="t_nav"><span>生活百般滋味，人生需要笑对。 </span><a href="${blog}/" class="n1">网站首页</a>
...
<c:forEach var="r" items="${list}">
    ...
    <span>作者：${r.createUser}</span><span>分类：[<a href="${blog}/chose.html?catCode=${r.catCode}">${r.articleName}</a>]</span><span>浏览：（${r.browse}）</span></p>
...
<aside class="right">
	...
	<form name="search_js1" method="post" action="${blog}/funny.html"> 
   ...
   <div class="rnav">
	...
	<c:forEach var="a" items="${articleTypeList}" varStatus="d">
   		 <li class="rnav${d.count}"><a href="${blog}/chose.html?catCode=${a.catCode}" >${a.articleName}</a></li>	  			
   	</c:forEach>
      </ul>           
    </div> 
<div class="label" id="divTags">
	...
	<li><a href="${blog}/chose.html?catCode=${parentCode}&label=${l.label}">${l.name}<span class="tag-count"> (${l.count})</span></a></li>
	</c:forEach>
    </ul>	 
</div>                
<div class="news"> 
