...
<form action="${blog}/voice/manage/videoUpdate.html" name="listForm" method="post" >
...
<div class="maincont othercolor">
  <div class="addmenu">
    ...
    <div class="add_meunlist">
      <ul>
        <li><span>标题：</span>
          <input type="text" placeholder="" name="title" id="title" value="${video.title}"/>
          <p class="error">* 标题不能为空</p>
        </li>
           <li><span>关键字：</span>
         <input type="text"  name="keyWord" id="keyWord" value="${video.keyWord}"/>
        </li>
            <li><span>封面图片：</span>
        	<c:if test="${not empty video.image}">
        		<div class="nowImg">
		        <div class="now_bar">				
			        <p class="file_name" title="${video.image}">${video.image}</p>
			        <span class="file_del" data-index="0" title="删除"></span>	
		        </div>
		        	<img src="/imageService/uploadFiles/${video.image}" id="${video.image}" class="tupics" />
		    </div>
        	</c:if>
       ...
       <li><span>是否发布：</span>
          <div class="sel_wrap">
          <label>请选择</label>
           <select class="form-control" name="isPublish">
	           <option value="1" <c:if test="${video.isPublish==1}"> selected="selected" </c:if>>是</option>
	           <option value="0" <c:if test="${video.isPublish==0}"> selected="selected" </c:if>>否</option>
       	   </select>
          </div>
        </li>
        <li><span>视频：</span>
        	<textarea style="width:90%;height:300px;" name="videoUrl" id ="workstatus">${video.videoUrl}</textarea>
        ...
        <a href="${blog}/voice/manage/videoList.html" class="btn danger">返回列表</a></li>
      </ul>
    </div>
  </div>
</div>
